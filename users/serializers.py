...
from shared.utility import check_email_or_phone, send_email, sen_phone_code, check_user_type
...
class SignUpSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        ...
        if user.auth_type == VIA_EMAIL:
            ...
            # sen_phone_code(user.phone_number code)
        user.save()
        return user
    ...
    def validate_email_phone_number(self, value):
        ...
        if value and User.objects.filter(email=value).exists():
            data = {
                "success" : False,
                "message" : "Bunday email bazada mavjud"
            }
            raise ValidationError(data)
        elif value and User.objects.filter(phone_number=value).exists():
            data = {
                "success": False,
                "message": "Bunday telefon raqam bazada mavjud"
            }
            raise ValidationError(data)
        return value
...
class ChangeUserInformation(serializers.Serializer):
    ...
    def validate_username(self, username):
        if len(username) < 5 or len(username) > 30:
            raise ValidationError(
                {
                    "message" : "Username must be betwen 5 and 30 characterd long"
                }
            )
        if username.isdigit():
            raise ValidationError(
...
class LoginSerializer(TokenObtainPairSerializer):
    ...
    def auth_validate(self, data):
        ...
        if check_user_type(user_input) == 'username':
            ...
            data = {
                "success" : False,
                "message" : "Siz faqat email, phone yoki username krita olsiz"
            }
            raise ValidationError(data)
        ...
        if current_user is not None and current_user.auth_type  in [NEW, CODE_VERIFIED]:
            raise ValidationError(
                {
                    "success" : False,
                    "message" : "Siz toliq ruyxatdan otmagansiz"
                }
            )
    ...
    def validate(self, attrs):
        ...
        if email_or_phone is None:
            raise ValidationError(
                {
                    "success" : False,
                    "message" : "Email or Phone required field"
                }
            )
...
class ResetPasswordSerializer(serializers.ModelSerializer):
    ...
    def validate(self, data):
        password = data.get('password', None)
        conform_password = data.get('confirm_password', None)

        if password != conform_password:
            raise ValidationError(
                {
                    "success" : False,
                    "message" : "Parolar mos emas"
                }
            )
        if password:
            validate_password(password)
